...
from calculer_coefficients import calculer_coefficients
...
class Affichage:
    ...
    def dessiner(self):
        for i in range(len(self.liste_points)-1):
            py.draw.line(self.fenetre,rouge,self.liste_points[i],self.liste_points[i+1],1)
            if not (self.liste_points[i][0],self.liste_points[i][1]) in self.distance_points.keys():
                self.distance_points[(self.liste_points[i][0],self.liste_points[i][1])]=np.sqrt(np.sum(self.liste_points[i]+self.liste_points[i+1]))
            epaisseur=2/self.distance_points[(self.liste_points[i][0],self.liste_points[i][1])]
