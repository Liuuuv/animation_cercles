...
class Affichage:
    ...
    for n in range(1, self.nb_cercles + 1):
        # Calculer les coefficients de Fourier pour un segment [-1, 1]
        # Les termes impairs pour une forme carrée : (4 / (n * π)) pour les termes impairs
        if n % 2 != 0:  # Seulement les termes impairs
            coefficient = (4 / (n * math.pi))
            self.liste_rayons.append(coefficient)
        else:
            self.liste_rayons.append(0)  # Les termes pairs sont nuls
