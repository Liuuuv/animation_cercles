...
class Affichage:
    ...
    def calculer_coefficients(self):
        for n in range(self.nb_cercles):
            resultat=scipy.integrate.quad(lambda t:self.fonction(t)*self.exp_complexe(-n*t),0,np.pi)[0] + 1j*scipy.integrate.quad(lambda t:self.fonction(t)*self.exp_complexe(-n*t),0,np.pi)[0]
            print(resultat)
            resultat/=2*np.pi
