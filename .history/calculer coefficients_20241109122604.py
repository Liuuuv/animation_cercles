...
image = io.imread('C:/Users/olivi/kDrive/projets/loisir/image/rui_tachibana.jpg')  # Remplace par le chemin de ton image
...
edges = feature.canny(smoothed_image, sigma=1)
...
# plt.scatter(np.real(ordered_contour_points), np.imag(ordered_contour_points), marker='.')

# # Ajouter une annotation avec le numéro de chaque point
# for idx, point in enumerate(ordered_contour_points):
#     plt.text(np.real(point), np.imag(point), str(idx), fontsize=8, color='red')

# plt.gca().invert_yaxis()
# plt.gca().set_aspect('equal', adjustable='box')
# plt.title("Contour de l'image dans le plan complexe avec l'ordre des points")
# plt.show()
