...
class Affichage:
    ...
    def calculer_coefficients(self):
        for n in range(self.nb_cercles):
            resultat,erreur=scipy.integrate.quad(lambda t:(self.fonction(t)*self.exp_complexe(-n*t)).real,0,np.pi).real + 1j* scipy.integrate.quad(lambda t:self.fonction(t)*self.exp_complexe(-n*t).real,0,np.pi)
            print(resultat)
