...
class Affichage:
    def __init__(self,facteur,suite,nb_cercles):
    ...
    def initialiser_cercles(self):
        ...
        # for n in range(-self.nb_cercles//2,self.nb_cercles//2):
        for n in range(0,self.nb_cercles):
            cercle=self.creer_cercle(self.suite(n),n)
            cercle.pos=pos[:]
            pos[0]+=cercle.rayon
...
# def u(n):
    # if n==0:
    #     return(0)
    # return 0.0001*math.floor(((phi**(nb_cercles-n))/math.sqrt(5))+1/2)
    # return (math.cos(n))*200
    # return (1/(nb_cercles-n))*100

def u(n):
    return [0,89,55,34,21,13,8,5,3,2,1,1][n]*2


facteur=0.8

affichage=Affichage(facteur,u,nb_cercles)
affichage.loop()