...
class Affichage:
    ...
    def calculer_coefficients(self):
        for n in range(self.nb_cercles):
            resultat,erreur=scipy.quad(lambda t:self.fonction(t)*self.exp_complexe(-t).real,0,math.pi).real + 1j* (scipy.quad(lambda t:self.fonction(t)*self.exp_complexe(-t).real,0,math.pi).imag, 0,math.pi)
