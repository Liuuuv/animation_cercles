...
class Affichage:
    ...
    def initialiser_cercles(self):
        ...
        for n in range(0,self.nb_cercles):
            cercle=self.creer_cercle(self.liste_rayons[n],n+1)
            cercle.pos=pos[:]
            pos[0]+=cercle.rayon
    ...
    def exp_complexe(self,t):
        return math.exp(1j*t)
    
    def calculer_coefficients(self):
        for n in range(self.nb_cercles):
            resultat,erreur=scipy.integrate.quad(lambda t:self.fonction(t)*self.exp_complexe(-n*t).real,0,math.pi).real + 1j* scipy.integrate.quad(lambda t:self.fonction(t)*self.exp_complexe(-n*t).real,0,math.pi)
            print(resultat)
