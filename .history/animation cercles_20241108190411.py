...
class Affichage:
    ...
    def calculer_coefficients(self):
        ...
        coefficients = np.fft.fft(x_values) / len(x_values)
        self.liste_rayons.append(abs(coefficients))
    
    
    def calculer_coefficients(self):
        # Définir un segment de -1 à 1 en points discrets
        x_values = np.linspace(-1, 1, num=self.nb_cercles)
        # Calculer la transformée de Fourier discrète
        self.coefficients = np.fft.fft(x_values) / len(x_values)
