import pygame as py
import math
import random as rd
import time
import scipy
import numpy as np
import scipy.integrate

py.init()



blanc=(255,255,255)
noir=(0,0,0)

rouge=(255,0,0)
vert=(0,255,0)
bleu=(0,0,255)


class Cercle:
    def __init__(self,rayon,vitesse):
        self.vitesse=vitesse
        self.rayon=rayon
        self.pos=None
        self.angle=0

class Marqueur:
    def __init__(self):
        self.pos=[0,0]



class Affichage:
    def __init__(self,facteur):
        self.dimensions=(int(1920*facteur),int(1080*facteur))
        self.fenetre=py.display.set_mode(self.dimensions)
        self.centre=(self.dimensions[0]//2+70,self.dimensions[1]//2)
        self.nb_cercles=nb_cercles


        self.marqueur=Marqueur()

        self.dt=1/120

        self.liste_cercles=[]
        self.liste_rayons=[]
        self.liste_points=[]
        
        self.calculer_coefficients()
        

        self.initialiser_cercles()


    def creer_cercle(self,rayon,vitesse):
        cercle=Cercle(rayon,vitesse)
        self.liste_cercles.append(cercle)
        return cercle

    def initialiser_cercles(self):
        pos=[self.centre[0],self.centre[1]]
        echelle=200
        for n in range(self.nb_cercles):
            cercle=self.creer_cercle(self.liste_rayons[n]*echelle,n)
            cercle.pos=pos[:]
            pos[0]+=cercle.rayon

    def mettre_a_jour_cercles(self):
        self.liste_cercles[0].angle+=self.liste_cercles[0].vitesse*self.dt
        for i in range(1,self.nb_cercles):
            angle_prec = self.liste_cercles[i - 1].angle
            rayon_prec = self.liste_cercles[i - 1].rayon

            # Position en X et Y en fonction du rayon et de l'angle du cercle précédent
            self.liste_cercles[i].pos[0] = self.liste_cercles[i - 1].pos[0] + rayon_prec * math.cos(angle_prec)
            self.liste_cercles[i].pos[1] = self.liste_cercles[i - 1].pos[1] + rayon_prec * math.sin(angle_prec)

            # Mise à jour de l'angle pour le mouvement du cercle courant
            self.liste_cercles[i].angle += self.liste_cercles[i].vitesse * self.dt

    def mettre_a_jour_marqueur(self):

        self.marqueur.pos[0]=self.liste_cercles[-1].pos[0]+self.liste_cercles[-1].rayon*math.cos(self.liste_cercles[-1].angle)
        self.marqueur.pos[1]=self.liste_cercles[-1].pos[1]+self.liste_cercles[-1].rayon*math.sin(self.liste_cercles[-1].angle)

        self.liste_points.append(self.marqueur.pos[:])



    def dessiner(self):
        for i in range(len(self.liste_points)-1):
            py.draw.line(self.fenetre,rouge,self.liste_points[i],self.liste_points[i+1],2)

        py.draw.circle(self.fenetre,bleu,self.marqueur.pos,2)

    def dessiner_cercles(self):
        for cercle in self.liste_cercles:
            py.draw.circle(self.fenetre,noir,cercle.pos,2)
            py.draw.circle(self.fenetre,noir,cercle.pos,abs(cercle.rayon),1)
    
    def fonction(self,t):
        if t<=math.pi:
            return (-2*t/math.pi)+1
        else:
            return (2*t/math.pi)-3
    
    def exp_complexe(self,t):
        return np.exp(1j*t)
    
    def calculer_coefficients(self):
        for n in range(self.nb_cercles):
            resultat=scipy.integrate.quad(lambda t:self.fonction(t)*self.exp_complexe(-n*t),0,np.pi)[0]
            resultat/=2*np.pi
            self.liste_rayons.append(abs(resultat))
            

    def loop(self):
        horloge=py.time.Clock()

        police=py.font.Font(None, 36)

        # boucle de jeu
        continuer=True
        # time.sleep(0.3)
        while continuer:
            for event in py.event.get():
                if event.type==py.QUIT:
                    continuer=False
                if event.type==py.KEYDOWN:
                    if event.key==py.K_ESCAPE:
                        continuer=False
            horloge.tick(120)
            py.display.set_caption(str(round(horloge.get_fps(),1)))


            self.fenetre.fill(blanc)

            self.dessiner_cercles()
            self.dessiner()

            self.mettre_a_jour_cercles()
            self.mettre_a_jour_marqueur()
            



            # texte_surface1 = police.render("number of circles: "+str(nb_cercles-1),False,noir)
            # self.fenetre.blit(texte_surface1,(655,200))

            py.display.flip()

        py.quit()


nb_cercles=50




facteur=0.8

affichage=Affichage(facteur)
affichage.loop()